...
public class OwenTesting extends SuperSmoothMover
{
    ...
    public OwenTesting()
    {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            //frames[i].scale(frames[i].getWidth()/4,frames[i].getHeight()/4);
    ...
    private void setItemXY(String productName)
    {
        ...
        { 
            itemX = 1010; itemY = 400; //lettuce bin area
        } 
        else if (productName.equals(SimulationWorld.PRODUCT_CARROT)) 
        { 
            itemX = 940; itemY = 400; //carrot bin area
        } 
        ...
        { 
            itemX = 800; itemY = 400; //orange bin area
        } 
        else if (productName.equals(SimulationWorld.PRODUCT_RAW_BEEF) )
        {
            itemX = 920; itemY = 280; //Display Warmer
        }
    ...
    private void retrieveItem(String productName)
    {
        ...
        if(item !=null)
        {
            ...
            basket.add(item);
getWorld().showText(item.getName()+" has "+item.getStock(),300,50);
getWorld().showText("basket size="+basket.size(),100, 50);
...
//getWorld().addObject(new SpeechBubble("total:"+total),300,300);
//getWorld().addObject(new SpeechBubble(text),100,100);
text=text+"\n"+"Total Item:"+count;
text=text+"\n"+"Total:"+Math.round(total*100.0)/100.0;

showText(text,Color.RED,300,150);
